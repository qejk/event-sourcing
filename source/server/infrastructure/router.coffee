class Space.eventSourcing.Router extends Space.Object
  ...
  _initializingMessageEventHandler: (event) ->
    @eventBus.subscribeTo event, (cmd) =>
      ...
      eventSourceable = @_handleDomainErrors(->
        ...
        if @_hasEventHandler(event)
          eventHandler = @_getEventHandler(event)
          eventHandler(event, instance)
        else
  ...
  _initializingMessageCommandHandler: (command) ->
    @commandBus.registerHandler command, (cmd) =>
      ...
      eventSourceable = @_handleDomainErrors(->
        ...
        if @_hasCommandHandler(command)
          commandHandler = @_getCommandHandler(command)
          commandHandler(cmd, instance)
        else
          instance.handle(cmd)
        return instance
      )
  ...
  _genericEventHandler: (event) =>
    ...
    eventSourceable = @_handleDomainErrors(->
      if @_hasEventHandler(event)
        eventHandler = @_getEventHandler(event)
        eventHandler(event, eventSourceable)
      else
  ...
  _genericCommandHandler: (command) =>
    ...
    eventSourceable = @_handleDomainErrors(->
      if @_hasCommandHandler(command)
        commandHandler = @_getCommandHandler(command)
        commandHandler(cmd, eventSourceable)
      else
  ...
  _hasEventHandler: (eventType) ->
    @handleEvents?()[eventType.toString()]?
  _getEventHandler: (eventType) ->
    @handleEvents?()[eventType.toString()] or null

  _hasCommandHandler: (commandType) ->
    @handleCommands?()[commandType.toString()]?
  _getCommandHandler: (commandType) ->
    @handleCommands?()[commandType.toString()] or null
